...
ddi <- read_ipums_ddi(paste0(path_cps,"/cps_00008.xml")) 
...
median_hourly_wage <- weighted.median(CPS_data$HOURWAGE[CPS_data$PAIDHOUR == 2 & CPS_data$HOURWAGE<900],
                                      w = CPS_data$ASECWT[CPS_data$PAIDHOUR == 2 & CPS_data$HOURWAGE<900], 
                                      na.rm = TRUE)
median_comp_percent <- 3/4 # How much of the difference between reported wage and the target wage will the gov. cover?
median_cutoff_percent <- 3/4 # What share of the median wage are we using to set the target wage? 
...
Impacted_IDs <- CPS_data  %>%
  ...
  select(SERIAL,YEAR,MONTH,CPSID,REPWT,ASECWT,AGE,INCWAGE,HOURWAGE,minwage.old,UHRSWORKT) %>% # keep only the variables that are needed
  mutate(Min_applied_wage = if_else(HOURWAGE>minwage.old,HOURWAGE,minwage.old), # Assuming no loss in hours, we raise everyone's hourly wage to the states minimum wage, if they are below the minimum wage. Note- we use 2023 rules, but data is from 2022 - so minwage.old
...
CPS_wage_subset_notreatment  <- CPS_data %>% 
  mutate(ruleYear = 2023,
         ...
         FilingStatus = 3) %>%
  mutate(hourwage_edit = if_else(between(WKSTAT,50,60),0,HOURWAGE)) %>% 
  filter(income_increase>0) %>%
  filter(RELATE==101) %>% 
  select(SERIAL,PERNUM,ASECWTH,STATEFIP,ruleYear,Year,numadults,numkids,famsize,FilingStatus,
...
#################
  ...
  ## Apply Benefits Calculator (block-by-block)----
...
#################
  ...
  treated_CPS <- CPS_wage_subset_treatment %>%
    mutate(net_benefit_receipt_with_subsidy = value.snap + 
             value.medicaid.adult + value.medicaid.child + value.aca +
             value.ctc +
             value.eitc +
             value.cdctc + 
    ...
    select(ASECWTH,SERIAL,income,famsize,numadults,numkids,
           agePerson1,agePerson2,agePerson3,agePerson4,agePerson5,agePerson6,agePerson7,agePerson8,
           net_benefit_receipt_with_subsidy,income_increase,
           value.snap_post,
...
test <- left_join(treated_CPS,Control_CPS) %>% 
  mutate(value.snap_change = value.snap_post - value.snap,
         ...
         diff_smart = if_else(net_benefit_receipt_with_subsidy - net_benefit_receipt<0,0,net_benefit_receipt_with_subsidy - net_benefit_receipt)) # Adjust so that someone would no have taken up the work if it pushed them over a cliff

# What are the aggregate trade offs across policies?
change_by_policy <- test %>% 
  select(SERIAL,ASECWTH,value.snap_change:value.cdctc_change) %>% 
  rename("SNAP" = "value.snap_change",
         "Medicaid (Adult)" = "value.medicaid.adult_change",
         "Medicaid (Child)" = "value.medicaid.child_change",
         "ACA" = "value.aca_change",
         "CTC" = "value.ctc_change",
         "EITC" = "value.eitc_change",
         "CDCTC" = "value.cdctc_change") %>% 
  gather(Policy,change,SNAP:CDCTC) %>% 
  group_by(Policy) %>% 
  summarize(Change = sum(ASECWTH*change, na.rm = TRUE))

change_by_policy %>% 
  ggplot(aes(x = Policy,
             y = Change/1000000,
             label = round(Change/1000000,0))) + 
  geom_hline(yintercept = 0) + 
  geom_point() +
  geom_text(vjust = 0, nudge_y = 10,check_overlap = TRUE) + 
  scale_y_continuous(labels = scales::dollar_format(prefix="$",suffix = " Million")) + 
  theme_bw() + 
  ylab("Change in Annual Safety Net Expenditure")

# For people who received some benefit chamge, what is the total benefit given what their benefit level was pre-subsidy?
test %>% 
  filter(diff != 0) %>% 
  ggplot(aes(x = net_benefit_receipt,
             y = diff)) +
  geom_hline( yintercept = 0) + 
  geom_point(aes(size = ASECWTH)) + 
  geom_smooth(aes(weight = ASECWTH)) + 
  scale_y_continuous(labels = scales::dollar_format(prefix="$")) + 
  scale_x_continuous(labels = scales::dollar_format(prefix="$")) +
  theme_bw() + 
  ylab("Change in Total Safety Net Receipt ($)") + 
  xlab("Social Safety Net Receipt Before Wage Subsidy ($)")

# Across pre-subsidy bins of social safety net receipt, what is the median change in social safety net $s?
test %>% 
  filter(diff != 0) %>% 
  mutate(net_benefit_receipt_round = round(net_benefit_receipt,-3)) %>% 
  na.omit() %>% 
  group_by(net_benefit_receipt_round) %>% 
  summarize(diff = weighted.median(diff, 
                                   w = ASECWTH,
                                   na.rm = TRUE)) %>% 
  ggplot(aes(x = net_benefit_receipt_round,
             y = diff)) +
  geom_hline( yintercept = 0) + 
  geom_point() + 
  geom_line() + 
...
test %>% 
  summarise(Number_of_households = sum(ASECWTH),
            Pre_sub_safetynetcost = sum(net_benefit_receipt*ASECWTH, na.rm = TRUE),
            post_sub_safetynetcost = sum(net_benefit_receipt_with_subsidy*ASECWTH, na.rm = TRUE),
            difference = sum(diff*ASECWTH, na.rm = TRUE),
            difference_Smart = sum(diff_smart*ASECWTH, na.rm = TRUE)) %>% 
